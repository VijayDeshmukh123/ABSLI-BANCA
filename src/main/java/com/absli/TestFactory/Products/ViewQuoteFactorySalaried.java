...
public class ViewQuoteFactorySalaried {
    ...
    public void ViewPageSalaried(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        viewquote.isSmokerSalariedTerm(testData.get("IsSmokerSalaried"));
        wait.WaitTime2();

        viewquote.enterSumAssuredWeb(testData.get("SumAssuredAmountSalaried"));
        wait.WaitTime2();

        if (testData.get("IsPlanOptionSalaried").equalsIgnoreCase("YES")) {
            viewquote.SelectPlanOption1(testData.get("PlanOptionSalaried"));
            wait.WaitTime2();
        }
        ...
        if (testData.get("IsPlanOptionSalaried").equalsIgnoreCase("YES")) {
            if (testData.get("PlanOptionSalaried").equalsIgnoreCase("Option 3 - Fixed Income Cover")) {
                wait.WaitTime2();
                viewquote.SelectIncomeBenefitPeriodWeb(testData.get("IncomeBenefitPeriod"));
            }
...
//        if(testData.get("IsACISalaried").equalsIgnoreCase("YES")){
//            viewquote.AddACIRiderWeb();
//            viewquote.enterSumAssuredACIWeb(testData.get("SumAssuredACISalaried"));
//        }
        ...
        // Add riders //
//        if (testData.get("IsAddRiderSalaried").equalsIgnoreCase("YES")) {
