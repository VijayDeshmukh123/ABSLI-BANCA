...
public class ViewQuoteFactoryVision_lifeincome {
    ...
    public void ViewPageVision_lifeincome(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        // View Quote page //
        if ((testData.get("IsSumAssured")).equalsIgnoreCase("YES")) {
            viewquote.EnterSumAssuredWeb(testData.get("SumAssured"));

        }
        if ((testData.get("IsPayingTerm")).equalsIgnoreCase("YES")) {
            viewquote.EnterPolpayingtermmWeb(testData.get("PayingTerm"));
        ...
        if (testData.get("IsAddRider").equalsIgnoreCase("YES")) {
            ...
            if (testData.get("Iswavierofpremium").equalsIgnoreCase("Waiver Of Premium")) {
                viewquote.addWOP();

            }
            if (testData.get("IsADDRider").equalsIgnoreCase("Accidental Death and Disability")) {
                viewquote.addADD(testData.get("ADDMamount"));
            }
            if (testData.get("IsCIRider").equalsIgnoreCase("Critical Illness")) {
                viewquote.addCI(testData.get("CIamount"));
            ...
            if (testData.get("IsHSRider").equalsIgnoreCase("Hospital Care")) {
                viewquote.addHC(testData.get("HCamount"));
                TestUtil.scrollTillEndOfPage(getDriver());
            }
            if ((testData.get("IsADBRider")).equalsIgnoreCase("Accidental Death Benefit Plus")) {
                viewquote.addADB(testData.get("ADBamount"));
            }
            viewquote.addbutton();
        ...
        softassert.assertEquals(PPTValue, testData.get("PayingTerm"));
