...
public class ViewQuoteFactoryN_Pension {
    ...
    public void ViewPageNischitPension(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        if (testData.get("IspolicytermN_Pension").equalsIgnoreCase("YES")) {
            viewquote.SelectPoltermWeb(testData.get("policytermN_Pension"));
        }
        if (testData.get("IsPaymodeN_Pension").equalsIgnoreCase("YES")) {
            viewquote.SelectPaymodeWeb(testData.get("paymodeN_Pension"));
        }
        if (testData.get("IsBenefitOptionN_Pension").equalsIgnoreCase("YES")) {
            viewquote.SelectbeneoptWeb(testData.get("benefitotionN_Pension"));
        }
        if (testData.get("IsBenefitfrequencyN_Pension").equalsIgnoreCase("YES")) {
            viewquote.SelectbenefreqWeb(testData.get("benefitfreqN_Pension"));
        }
        if (testData.get("IsIncomeVarientN_Pension").equalsIgnoreCase("YES")) {
            viewquote.SelectincomevarWeb(testData.get("IncomeVariantN_Pension"));
        }
        if (testData.get("IsDefermenntPeriodN_Pension").equalsIgnoreCase("YES")) {
            viewquote.SelectidefermentperiodWeb(testData.get("DefermentPeriodN_Pension"));
        }
        if (testData.get("IsSAmultiplierN_Pension").equalsIgnoreCase("YES")) {
            viewquote.SelectisummultiplierWeb(testData.get("SumAssuredMultiplierN_Pension"));
        }

        // Add riders //
        if (testData.get("IsAddRiderN_Pension").equalsIgnoreCase("YES")) {
            viewquote.addriderbutton();

            if (testData.get("IsADBRiderN_Pension").equalsIgnoreCase("Accidental Death Benefit Plus")) {
                viewquote.addADB(testData.get("ADMamountN_Pension"));
            }
            if (testData.get("IsCIRiderN_Pension").equalsIgnoreCase("Critical Illness")) {
                viewquote.addCI(testData.get("CIamountN_Pension"));
                TestUtil.scrollTillEndOfPage(getDriver());
            }
            if (testData.get("IsSCRiderN_Pension").equalsIgnoreCase("Surgical Care")) {
                viewquote.addSC(testData.get("SCamountN_Pension"));
            }
            if (testData.get("IsHSRiderN_Pension").equalsIgnoreCase("Hospital Care")) {
                viewquote.addHC(testData.get("HCamountN_Pension"));
            }
            if (testData.get("IswavierofpremiumN_Pension").equalsIgnoreCase("Waiver Of Premium")) {
                viewquote.addWOP();
            }
            viewquote.addbutton();
        }
