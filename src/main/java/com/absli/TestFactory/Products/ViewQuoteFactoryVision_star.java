...
public class ViewQuoteFactoryVision_star {
    ...
    public void ViewPageVision_star(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        viewquote.enterSumAssuredWeb(testData.get("SumAssuredAmount"));
        wait.WaitTime2();

        if (testData.get("IsPlanOption").equalsIgnoreCase("YES")) {
            viewquote.SelectPlanOptionweb(testData.get("PlanOption"));
            wait.WaitTime2();
        }
        ...
        if (testData.get("IsPaymode").equalsIgnoreCase("YES")) { // For Single pay only Annual is there.
