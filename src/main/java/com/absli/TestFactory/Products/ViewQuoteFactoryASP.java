...
public class ViewQuoteFactoryASP {
    ...
    public void ViewPageASP(WebDriver driver, HashMap<String, String> testData) throws InterruptedException, IOException {
        ...
        // Add riders //
        if (testData.get("IsAddRiderASP").equalsIgnoreCase("YES")) {
            viewquote.addriderbutton();

            if (testData.get("IswavierofpremiumASP").equalsIgnoreCase("Waiver Of Premium")) {
                viewquote.addWOP();

            }
            if (testData.get("IsADBRiderASP").equalsIgnoreCase("Accidental Death Benefit Plus")) {
                viewquote.addADB(testData.get("ADMamountASP"));
            }
            if (testData.get("IsCIRiderASP").equalsIgnoreCase("Critical Illness")) {
                viewquote.addCI(testData.get("CIamountASP"));
                TestUtil.scrollTillEndOfPage(getDriver());
            }
            if (testData.get("IsSCRiderASP").equalsIgnoreCase("Surgical Care")) {
                viewquote.addSC(testData.get("SCamountASP"));
            }
            if (testData.get("IsHSRiderASP").equalsIgnoreCase("Hospital Care")) {
                viewquote.addHC(testData.get("HCamountASP"));
                TestUtil.scrollTillEndOfPage(getDriver());
            }
            viewquote.addbutton();
